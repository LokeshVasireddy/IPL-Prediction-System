...
def predict_match():
    ...
    for key2, value2 in team.items():
        if teamB == key2:
            ...
            if team2_score < team1_score:
                result["teamB"][
                    "match_result"
                ] = f"{steam[str(teamA)]} won by {round(float(pred1[0, 0] - float(pred2[0, 0])))} runs"
                winner = key1
                team2_scored = team2_score
            else:
                team2_scored = math.ceil(
                    round(team1_score / team2_run_rate, 1) * team2_run_rate
                )
                ...
                result["teamB"][
                    "match_result"
                ] = f"{value2} chased down the score in {oversface}.{remaining_balls} overs"
                winner = key2
    ...
    threading.Thread(
        target=train_model_with_new_data, args=(batfirst, batsecond)
    ).start()
